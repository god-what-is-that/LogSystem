        ...
        if (item === "group_id") {
          if (match[item] == "此条来自xt数据库，没有group_id") {
            ...
            if (matchResult) {
              ...
              // 设置头像
              const avatarUrl = `/static/images/groups/${qqNum}.jpg`;
...
class InitializeEditForm {
  ...
  initFieldCheck() {
    ...
    form.addEventListener("click", (event) => {
      ...
      // 点击按钮关闭表单
      if (event.target.tagName === "BUTTON" && event.target.id === "close-form" && this.validationStates.get("upload") !== false) {
        event.preventDefault()
  ...
  handleSubmit (form) {
    ...
    input.value = JSON.stringify(this.packMatch(form))
